# Your first program - finished project

The completed code from the [Your first program](https://scratch4j.openpatch.org/tutorials/getting-started)
tutorial.
...
**BlueJ** — open this folder, then right-click `MyStage` and choose `new MyStage()`.

**VS Code** — open this folder. The `.vscode` settings already point at `+libs`,
so press the Run button above `main` in `MyStage.java`.

**A terminal** — from inside this folder:

```
javac -cp "+libs/*" -d . *.java
java -cp "+libs/*:." MyStage
```

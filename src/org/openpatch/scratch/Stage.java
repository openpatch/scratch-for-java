...
public class Stage {
  ...
  /**
   * <<<<<<< HEAD Adds a new shader to the sprite. If a shader with the received name already
   * exists, this method ======= Sets the texture sampling mode Point sampling: both magnification
   * and minification filtering are set to nearest. Linear sampling: magnification filtering is
   * nearest, minification set to linear Bilinear sampling: both magnification filtering is set to
   * linear and minification either to linear-mipmap-nearest (linear interpolation is used within a
   * mipmap, but not between different mipmaps). Trilinear sampling: magnification filtering set to
   * linear, minification to linear-mipmap-linear, which offers the best mipmap quality since linear
   * interpolation to compute the value in each of two maps and then interpolates linearly between
   * these two values.
   *
   ...
   * >>>>>>> d7b76036d7d2175245fa8de40d42d972914b35f6 does nothing.
   *
   * @param name
   * @param shaderPath
   * @return the shader

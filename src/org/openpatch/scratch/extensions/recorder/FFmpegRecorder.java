 ...
 *
 * <p>This class creates a temporary directory to store PNG files for each frame and uses FFmpeg to
 * convert these frames into a video file when the recording is stopped.
 *
 * <p>Usage example:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>Note: Ensure that FFmpeg is installed and available in the system's PATH.
 */
public class FFmpegRecorder extends Recorder {
   ...
   *     <p>This constructor initializes the FFmpegRecorder by creating a temporary directory and
   *     setting up a pattern for temporary PNG files. If the temporary directory cannot be created,
   ...
   *
   * <p>This method overrides the `stop` method from the superclass. It first calls the superclass's
   ...
   *
   * <p>The FFmpeg command is executed, and a message is printed to the console indicating that the
   * video conversion is in progress. If there is an error during the execution of the command, the

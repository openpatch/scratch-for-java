...
public class Window {
   ...
   *
   * <p>Point sampling: both magnification and minification filtering are set to nearest. Linear
   * sampling: magnification filtering is nearest, minification set to linear Bilinear sampling:
   * both magnification filtering is set to linear and minification either to linear-mipmap-nearest
   * (linear interpolation is used within a mipmap, but not between different mipmaps). Trilinear
   * sampling: magnification filtering set to linear, minification to linear-mipmap-linear, which
   * offers the best mipmap quality since linear interpolation to compute the value in each of two
  ...
  private Runnable whenExitsHandler;
  ...
  public void whenExits() {
    this.whenExitsHandler.run();
  }

  public void setWhenExits(Runnable runnable) {
